...
class pretrained_resnet_architecture:
  ...
  def __call__(self,inputs,input_y, classes, training):
    ...
    self.variables = tf.contrib.framework.get_variables("resnet_v1_50/logits") + tf.contrib.framework.get_variables("resnet_v1_50/AuxLogits")
...
class resnet50():
  def __init__(self,data_format='channels_last',name='resnet50',trainable=True,include_top=True,pooling=None,classes=1000):
    self.reuse = tf.AUTO_REUSE
  ...
  def __call__(self, input_tensor_batch,input_y,training=True):
    with tf.variable_scope(self.name, reuse = self.reuse):
        input_tensor_batch_3_channel = tf.concat([input_tensor_batch, input_tensor_batch,input_tensor_batch], axis=3)
