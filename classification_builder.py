...
class ExperimentBuilder(object):
    def __init__(self, parser, data):
        ...
        if self.pretrain>0:
            self.input_y = tf.placeholder(tf.float32, [self.num_gpus, self.batch_size, self.data.training_classes], 'y_inputs-1')
        else:
            self.input_y = tf.placeholder(tf.float32, [self.num_gpus, self.batch_size, self.data.testing_classes], 'y_inputs-1')
        self.input_x_j = tf.placeholder(tf.float32, [self.num_gpus, self.batch_size, self.support_number ,image_height, image_width,
        ...
        if self.pretrain>0:
            classifier = densenet_classifier(input_x_i=self.input_x_i, input_y=self.input_y, classes=self.data.training_classes, 
            ...
            classifier = densenet_classifier(input_x_i=self.input_x_i, input_y=self.input_y, classes=self.data.testing_classes, 
                    batch_size=self.batch_size, layer_sizes=self.discriminator_layers, inner_layers=self.discr_inner_layers, num_gpus=self.num_gpus, 
                    use_wide_connections=self.use_wide_connections,
                    is_training=self.training_phase, augment=self.random_rotate,dropout_rate=self.dropout_rate)
                ...
                for e in range(self.classification_total_epoch):
                    ...
                    with tqdm.tqdm(total=self.total_test_batches) as pbar_samp:
                        for iter in range(int(self.total_test_batches)):
                            if self.pretrain>0:
                                x_train_classification_i, x_train_i,_,_,y_test,y_train = self.data.get_train_batch()
                            else:
                                x_train_classification_i, x_train_i,_,_,y_test,y_train = self.data.get_test_batch()

                            x_train_classification_i = np.reshape(x_train_classification_i,[x_train_classification_i.shape[0],x_train_classification_i.shape[1]*x_train_classification_i.shape[2],\
                                x_train_classification_i.shape[3],x_train_classification_i.shape[4],x_train_classification_i.shape[5]])

                            x_train_i = np.reshape(x_train_i,[x_train_i.shape[0],x_train_i.shape[1]*x_train_i.shape[2],\
                                x_train_i.shape[3],x_train_i.shape[4],x_train_i.shape[5]])
                            ...
                            test_loss_value, test_acc_value, test_summary = sess.run([self.losses["losses"], self.accuracy,self.summary],
                                feed_dict={self.input_x_i: x_train_classification_i,
                                           self.input_y: y_test,
                                           self.dropout_rate: self.dropout_rate_value,
                        ...
                        iter_out = "{},total_test_loss: {}, total_test_accuracy: {} ".format(iter,total_test_loss_mean,total_test_accuracy_mean)
