...
def sample_generator(num_generations, sess, same_images,  dropout_rate, dropout_rate_value, data, batch_size,file_name,
    ...
    input_images, support_images, generated, few_shot_y_label, y_label, similarities,f_encode_z,matching_feature = sess.run(same_images, feed_dict={
    ...
    support_list = np.zeros(shape=(batch_size, num_generations, support_images.shape[-4] ,support_images.shape[-3], support_images.shape[-2],
    ...
    if is_interpolation:
        ...
        for i in range(num_generations):
            input_images, support_images, generated, few_shot_y_label, y_label, similarities,f_encode_z,matching_feature = sess.run(same_images, feed_dict={z_input: batch_size*[z_vectors[i]],
                                                                        input_a:conditional_inputs, 
                                                                        input_b:support_input, 
                                                                        input_y_i:y_input_i, 
                                                                        input_y_j:y_input_j, 
    ...
    input_images, support_images, generated = data.reconstruct_original(input_images_list), data.reconstruct_original(support_list), data.reconstruct_original(generated_list)
    images = np.zeros(shape=(batch_size, 1+support_num+num_generations,support_images.shape[-3], support_images.shape[-2],
                                     support_images.shape[-1]))
...
def sample_generator_for_classifier(num_generations, iteration, sess, same_images,  
    ...
    augmented_support_image = np.zeros([batch_size,(augmented_number+data.support_number)*data.selected_classes,height,height,channel])
    augmented_few_shot_label = np.zeros([batch_size,(augmented_number+data.support_number)*data.selected_classes,data.selected_classes])
    augmented_global_label = np.zeros([batch_size,(augmented_number+data.support_number)*data.selected_classes,classes_number])
    ...
    for j in range(data.selected_classes):
        ...
        y_global_input_j_selected = y_global_input_j[:,:,j*data.support_number:j*data.support_number+support_number,:]
        ...
        for i in range(augmented_number):
            ...
            input_images, support_images, generated, few_shot_y_batch, y_batch_r, few_shot_y_support, y_support, similarities, \
            d_loss,  few_shot_fake_category, few_shot_confidence_score = sess.run(same_images, feed_dict={z_input: batch_size*[z_vectors_current[0]],
                                                                        input_a:x_batch, 
                                                                        input_b:support_input_selected, 
                                                                        input_y_i:y_batch, 
                                                                        input_y_j:y_input_j_selected, 
        ...
        if iteration == 0:
            ...
            for j in range(data.selected_classes ):
                for s in range(file_num):
                    current_images = unstack(augmented_support_image[:,augmented_number*j + 32*s + data.support_number*data.selected_classes : augmented_number*j + 32*(s+1) + data.support_number*data.selected_classes])
                    ...
                    current_images = (current_images - np.min(current_images)) / (np.max(current_images) - np.min(current_images))
...
def sample_generator_selector(num_generations, sess, same_images,  
    ...
    for i in range(num_generations):
        input_images, support_images, generated, few_shot_y_batch, y_batch, few_shot_y_support, y_support, similarities, \
        d_loss,  few_shot_fake_category, few_shot_confidence_score = sess.run(same_images, feed_dict={z_input: batch_size*[z_vectors[i]],
                                                                    input_a:conditional_inputs, 
                                                                    input_b:support_input, 
                                                                    input_y_i:y_input_i, 
                                                                    input_y_j:y_input_j, 
    ...
    if is_saved: 
        images = np.zeros(shape=(batch_size, 1+ selected_classes*(support_number+augmented_number), support_images.shape[-3], support_images.shape[-2],
                                         support_images.shape[-1]))    
        ...
        images = np.zeros(shape=(batch_size, selected_classes*(support_number+augmented_number), support_images.shape[-3], support_images.shape[-2],
                                         support_images.shape[-1]))
    ...
    for k in range(batch_size):
      for i in range(selected_classes):
        ...
        images[k,1+selected_classes*support_number+i*augmented_number:1+selected_classes*support_number+(i+1)*augmented_number] = data.reconstruct_original(np.array(current_category_images[:augmented_number]))
        few_shot_label[k,1+selected_classes*support_number+i*augmented_number:1+selected_classes*support_number+(i+1)*augmented_number] = current_category_fewshot_label[k]
        global_label[k,1+selected_classes*support_number+i*augmented_number:1+selected_classes*support_number+(i+1)*augmented_number] = current_category_global_label[k]
...
def sample_generator_selector_on_batch(num_generations, iteration, sess, same_images,  
    ...
    generated_fewshot_label = np.zeros([batch_size,data.selected_classes,num_generations,few_shot_y_support.shape[-1]])
    selected_fewshot_label = np.zeros([batch_size,data.selected_classes,augmented_number,few_shot_y_support.shape[-1]])
    #### genrate image and labels for each batch image
    ...
    for i in range(data.selected_classes):
        ...
        selected_images[:,i] = np.reshape(generated_images[:,i][mask_index[:,0],mask_index[:,1]],(batch_size,augmented_number,height,height,channel)) 
        selected_fewshot_label[:,i] =  np.reshape(generated_fewshot_label[:,i][mask_index[:,0],mask_index[:,1]],(batch_size,augmented_number,data.selected_classes)) 
        selected_global_label[:,i] =  np.reshape(generated_global_label[:,i][mask_index[:,0],mask_index[:,1]],(batch_size,augmented_number,classes_number)) 
        
        if iteration==0:
            current_augmented_image = np.zeros([batch_size,data.support_number + augmented_number, height,height,channel])
            current_augmented_image[:,:data.support_number,:,:,:] = support_input[:,:,j*data.support_number:(j+1)*data.support_number,:,:,:]
    ...
    augmented_support_image[:,data.support_number*data.selected_classes:] = np.reshape(selected_images,(batch_size,selected_images.shape[1]*selected_images.shape[2],height,height,channel))
    augmented_few_shot_label[:,data.support_number*data.selected_classes:] = np.reshape(selected_fewshot_label,(batch_size,selected_fewshot_label.shape[1]*selected_fewshot_label.shape[2],data.selected_classes))
    augmented_global_label[:,data.support_number*data.selected_classes:] = np.reshape(selected_global_label,(batch_size,selected_global_label.shape[1]*selected_global_label.shape[2],classes_number))
...
def sample_two_dimensions_generator(sess, same_images,
                                    ...
                                    batch_size, file_name, conditional_inputs ,input_a,support_input,input_b,input_y, y_input, classes_number, classes,
    ...
    input_images,support_images, generated,y_label,similarities = sess.run(same_images, feed_dict={input_a:conditional_inputs, 
                                                                input_b:support_input, 
                                                                input_y:y_input, 
                                                                classes:classes_number,
                                                                 dropout_rate: dropout_rate_value,
    ...
    for i in range(num_generations):
        input_images, support_images, generated,y_label,similarities = sess.run(same_images, feed_dict={z_input: batch_size*[z_vectors[i]],
                                                                    z_input_2: batch_size*[z_vectors_2[i]],
                                                                      input_a:conditional_inputs, 
                                                                      input_b:support_input,
                                                                      input_y:y_input, 
