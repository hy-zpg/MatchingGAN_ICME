...
def one_channel_preparation(data_dir,dataset,length,channels,batch_size):
    train_loader_1 = FIGR_Omniglot(data_dir, dataset,transform=transforms.Compose([lambda x: Image.open(x).convert('L'),
...
def one_channel_evaluation(data_dir,dataset,length,channels=1):
    dset = FIGR_Omniglot(data_dir, dataset, transform=transforms.Compose([lambda x: Image.open(x).convert('L'),
                                                                   lambda x: x.resize((length,length)),
                                                                   lambda x: np.reshape(x, (length, length, channels)), 
...
def three_channel_evaluation(data_dir,dataset,length,channels=1):
    ...
    dset = FIGR_Omniglot(data_dir, dataset, transform=transforms.Compose([lambda x: cv2.imread(x),
                                                                        lambda x: cv2.resize(x,(length,length), interpolation=cv2.INTER_LINEAR)
...
class generate_image_label_pairs():
    def __init__(self, dataroot, dataset, image_size, channels=1, each_class_total_samples=8):
        ...
        save_file_data = self.npy_file + '{}_c{}_s{}_data.npy'.format(self.dataset,classes,self.image_size)
        ...
        if not os.path.isfile(save_file_data):
            ...
            # self.dataloader = np.array([self.dataloader[i][:self.each_class_total_samples,:,:,:] for i in shuffle_classes if np.shape(self.dataloader[i])[0] >= self.each_class_total_samples])
...
def flowers(image_dir, label_dir, save_dir):
    ...
    for index_real, item in enumerate(flower_labels):
        ...
        if os.path.exists(save_dir+'/{}'.format(item)):
            flag = int(index / 10)
            if flag<1:
                shutil.move(image_dir+'image_0000{}.jpg'.format(index),save_dir+'/{}/image_0000{}.jpg'.format(item,index))
            elif 1<=flag <10:
                shutil.move(image_dir+'image_000{}.jpg'.format(index),save_dir+'/{}/image_000{}.jpg'.format(item,index))
            elif 10<=flag <100:
                shutil.move(image_dir+'image_00{}.jpg'.format(index),save_dir+'/{}/image_00{}.jpg'.format(item,index))
            elif 100<=flag <1000:
                shutil.move(image_dir+'image_0{}.jpg'.format(index),save_dir+'/{}/image_0{}.jpg'.format(item,index))
        else:
...
dataroot = './coarse-data/visualization_training_images/vggface'
dataset = 'test_vggface'
generate_image_label_pairs(dataroot=dataroot, dataset=dataset, image_size=84, channels=3, each_class_total_samples=3)
